...
async function validateSubscription(): Promise<void> {
  ...
  const upstream = "actions-rs/cargo";
...
async function ensureCross(): Promise<string> {
  try {
    return await io.which("cross", true);
  } catch {
    core.info(
      "`cross` not found in PATH; installing via `cargo install cross`..."
    );
...
export async function run(actionInput: input.Input): Promise<void> {
  const program = actionInput.useCross
    ? await ensureCross()
